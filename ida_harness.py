...
import hashlib
import glob 
from checker_mp_save_model import * 
import r2_cfg
import run_objdump
import pickle 
...
def call_ida(binary,check_addr,function_name):
    ...
    try:
        ...
        return None , None  
    p=["/mnt/c/Program\ Files/IDA\ 7.3/idat.exe","-A","-S\"","./ida_binaryai_linux.py",str(addr),"\"",binary]
    #out = check_output(p,stderr=subprocess.STDOUT,timeout=60)
    try:
        st = os.system(" ".join(p))
    except:
        return None , None  
    if st==0 :
        try:
            score = open(binary+"_output.json",'r').read()
            score = json.loads(score)
        except:
            return None , None  
        adr = list(score.keys())[0]
        rank = None 
        similarity = None 
        for ixxx in range(len(score[adr]['full'])):
            if "coreutils" in score[adr]['full'][ixxx]['function']['sourceFile']:
                if score[adr]['full'][ixxx]['function']['name'].lower() == function_name.lower():
                    rank = ixxx+1  
                    similarity = score[adr]['full'][ixxx]['score']
                    os.system('rm '+binary+"_output.json")
                    os.system('rm '+binary+".idb")
...
def wrapper(MAIN_SEED,SAVE_PATH,function_name):
    ...
    def main(function_name,NUM_GEN):
        ...
        for items in gen_1:
            ...
            score,grad = call_ida(items,check_addr,function_name)
...
if __name__ == "__main__":
    ...
    MAIN_SEED = "./uroboros_testing/bin_bk/"+binary_seed
    SAVE_PATH ="./uroboros_testing/function_container_"+function_name+"_"+MAIN_SEED.split("/")[-1]
