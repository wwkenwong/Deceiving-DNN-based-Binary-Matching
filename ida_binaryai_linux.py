...
class BinaryAIManager(object):
    ...
    def client(self):
        if self._client is None:
            try:
                bai_config['token'] = random.choice(POOL_OF_KEY)
...
def cmd_match(funcset_ids=None):
    ...
    for ea in idautils.Functions():
        try:
            ...
            targets, func_id = bai_mgr.retrieve(ea, 500, funcset_ids, 2)            
        except Exception as e:
            print(str(e))
            continue

        if targets and func_id:
            gg = copy.copy(targets)
            for iii in range(0,len(gg)):
                # dict_keys(['id', 'language', 'name', 'sourceCode', 'sourceFile', 'sourceLine'])
                tmp = gg[iii]['function'].pop('language')
                tmp = gg[iii]['function'].pop('sourceCode')
                tmp = gg[iii]['function'].pop('sourceLine')
    ...
    a = ida_nalt.get_input_file_path()
    folder = "\\".join(a.split("\\"))#[:-1])
    #path = os.path.join(folder, "output.json")
    path = folder+"_output.json"
...
def match_by_addr(ea,funcset_ids=None):
    bai_mgr = BinaryAIManager()
    output_json = {}
    addr_list = list(idautils.Functions())
    addr_list = [int(x) for x in addr_list]
    if ea in addr_list:
        try:
            #targets, func_id = bai_mgr.retrieve(ea, bai_config['topk'], funcset_ids, 2)
            targets, func_id = bai_mgr.retrieve(ea, 500, funcset_ids, 2)            
